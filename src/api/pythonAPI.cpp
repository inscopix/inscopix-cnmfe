...
PYBIND11_MODULE(inscopix_cnmfe, handle)
{
    ...
    output_dir_path (str): Path to the output directory (output files not saved when given an empty string)
    ...
    min_pixel_correlation (float): Minimum pixel correlation when searching for seed pixels
    min_peak_to_noise_ratio (float): Minimum peak-to-noise ratio when searching for seed pixels
    ...
    ring_size_factor (float): Multiple of the average cell diameter to use for computing the radius of the ring model used for estimating the background activity
    merge_threshold (float): Temporal correlation threshold for merging cells that are spatially close
    num_threads (int): Number of threads to use for processing
    processing_mode (int): Processing mode to use to run CNMF-E (0: all in memory, 1: sequential patches, 2: parallel patches)
    patch_size (int): Side length of an individual square patch of the field of view in pixels
    patch_overlap (int): Amount of overlap between patches in pixels
    output_units (int): Output units for the temporal components (0: dF, 1: noise scaled)
    ...
    py::arg("min_pixel_correlation") = 0.8,
    py::arg("min_peak_to_noise_ratio") = 10.0,
